#!/system/bin/sh

THEME_FILE="/data/adb/modules/OpenList/data/theme.conf"
mkdir -p "$(dirname "$THEME_FILE")"
if [ ! -f "$THEME_FILE" ]; then
  echo "theme=light" > "$THEME_FILE"
fi

grep '^theme=' "$THEME_FILE" | cut -d'=' -f2

MODDIR="/data/adb/modules/OpenList"
LOG="$MODDIR/log.log"

log() {
  echo "[$(date '+%F %T')] $1" >> "$LOG" 2>/dev/null || true
}

is_running() {
  pgrep -f 'openlist server' >/dev/null
}

start_once() {
  local bin="$MODDIR/bin/openlist"
  [ -x "$bin" ] || return 1

  chmod 755 "$bin" 2>/dev/null
  chcon u:object_r:system_file:s0 "$bin" 2>/dev/null || true

  "$bin" server --data "$MODDIR/data" >/dev/null 2>&1 &

  local i=0
  while [ $i -lt 5 ]; do
    is_running && {
      log "启动 OpenList 服务"
      return 0
    }
    sleep 0.2
    i=$((i + 1))
  done
  return 1
}

delete_large_log() {
  [ -f "$LOG" ] || return 0
  log_size=$(wc -c <"$LOG" 2>/dev/null || echo 0)
  if [ "$log_size" -gt 1048576 ]; then
    rm -f "$LOG"
    log "日志文件已清理，因大小超过 1MB"
  fi
}

get_ip_port() {
  local ip port cfg="$MODDIR/data/config.json"

  ip=$(sh -c '
        ip -o -4 addr show 2>/dev/null |
        awk -F"[ /]+" "/inet / && \$4 != \"127.0.0.1\" {print \$4; exit}" ||
        ip route get 1 2>/dev/null |
        awk -F"src " "/src / {print \$2; exit}"
    ')

  [ -z "$ip" ] && ip="127.0.0.1"
  [ -r "$cfg" ] &&
    port=$(awk -F'[:"[:space:]]+' '/"http_port"/ {print $3}' "$cfg" 2>/dev/null | tr -d ', ')
  [ -z "$port" ] && port=5244

  echo "${ip}:${port}"
}

write_prop() {
  local ip_port=$(get_ip_port)
  local status=$(is_running && echo "运行中" || echo "已停止")
  local desc="服务:${status} | 本机设备IP:${ip_port}"
  sed -i "s/^description=.*/description=$desc/g" "$MODDIR/module.prop" 2>/dev/null || true
}

MAX_ATTEMPTS=5
attempt_count=0

write_prop

last_ip=""

while [ $attempt_count -lt $MAX_ATTEMPTS ]; do
  if [ -x "$MODDIR/bin/openlist" ]; then
    if ! is_running; then
      start_once
      if [ $? -ne 0 ]; then
        attempt_count=$((attempt_count + 1))
        if [ $attempt_count -eq $MAX_ATTEMPTS ]; then
          log "OpenList 服务启动失败，已尝试 $MAX_ATTEMPTS 次，停止启动服务"
          break
        fi
      else
        attempt_count=0
        write_prop
      fi
    fi
  fi

  delete_large_log

  current_ip=$(get_ip_port | cut -d':' -f1)
  if [ "$current_ip" != "$last_ip" ] && [ -n "$current_ip" ]; then
    write_prop
    last_ip="$current_ip"
  fi

  sleep 1
done